...
public class InAppNotificationService : IInAppNotificationService, IDisposable
{
    ...
    {
        await SendNotificationAsync(TranslationHelper.GetString(Language.NotificationTitleInfo), message,
            NotificationType.Information);
    }

    public async Task SendSuccessNotificationAsync(string message)
    {
        await SendNotificationAsync(TranslationHelper.GetString(Language.NotificationTitleSuccess), message,
            NotificationType.Success);
    }

    public async Task SendWarningNotificationAsync(string message)
    {
        await SendNotificationAsync(TranslationHelper.GetString(Language.NotificationTitleWarning), message,
            NotificationType.Warning);
    }

    public async Task SendErrorNotificationAsync(string message)
    {
        await SendNotificationAsync(TranslationHelper.GetString(Language.NotificationTitleError), message, NotificationType.Error);
    }

    public void SendInfoNotificationSync(string message)
    {
        SendNotificationSync(TranslationHelper.GetString(Language.NotificationTitleInfo), message, NotificationType.Information);
    }

    public void SendSuccessNotificationSync(string message)
    {
        SendNotificationSync(TranslationHelper.GetString(Language.NotificationTitleSuccess), message, NotificationType.Success);
    }

    public void SendWarningNotificationSync(string message)
    {
        SendNotificationSync(TranslationHelper.GetString(Language.NotificationTitleWarning), message, NotificationType.Warning);
    }

    public void SendErrorNotificationSync(string message)
    {
        SendNotificationSync(TranslationHelper.GetString(Language.NotificationTitleError), message, NotificationType.Error);
    }
