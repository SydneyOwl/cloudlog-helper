...
public class HRDLogThirdPartyLogService : ThirdPartyLogService
{
    ...
    [UserInput("Upload rig info", InputType = FieldType.CheckBox)]
    ...
    public override async Task UploadQSOAsync(string? adif, CancellationToken token)
    {
        ...
        if (errorElement?.Value is not null) throw new Exception($"Error uploading qso: {errorElement.Value.Replace("\n","").Trim()}");
