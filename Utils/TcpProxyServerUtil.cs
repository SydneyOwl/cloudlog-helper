...
{
    public class TcpProxyServerUtil
    {
        ...
        private static async Task HandleClientAsync(TcpClient client, Func<string,Task<string>> callback, CancellationToken ct)
        {
            try
            {
                using (client)
                await using (var stream = client.GetStream())
                {
                    while (true)
                    {
                        ...
                        if (bytesRead > 0)
                        {
                            ...
                            var st = System.Text.Encoding.ASCII.GetString(request);
