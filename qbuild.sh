...
dotnet publish -c Release -r win-x64 -f net6.0-windows10.0.17763.0 -p:UseAppHost=true -p:PublishSingleFile=true --self-contained true -p:PublishReadyToRun=true -p:PublishTrimmed=true -p:IncludeNativeLibrariesForSelfExtract=true
...
dotnet publish -c Release -r win-x86 -f net6.0-windows10.0.17763.0 -p:UseAppHost=true -p:PublishSingleFile=true --self-contained true -p:PublishReadyToRun=true -p:PublishTrimmed=true -p:IncludeNativeLibrariesForSelfExtract=true
zip -rj bin/CloudlogHelper-v${tagName}-windows-x86.zip bin/Release/net6.0/win-x86/publish/CloudlogHelper.exe


# build linux x64
dotnet restore -r linux-x64
dotnet publish -c Release -r linux-x64 -f net6.0 -p:PublishSingleFile=true -p:UseAppHost=true --self-contained true -p:PublishReadyToRun=true -p:PublishTrimmed=true -p:IncludeNativeLibrariesForSelfExtract=true
