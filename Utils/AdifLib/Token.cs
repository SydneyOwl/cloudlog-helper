...
public class Token
{
    ...
    private bool _isHeader; // Flag that indicates whether this is a header.
    ...
    public Token(string TokenString, bool IsHeader)
    {
        ...
        if (TokenString.Trim() != "") /* if empty string is passed, assume no initialization */ ParseToken(TokenString);
    }
    ...
    public Token(string TagName, string Data, char DataType, string Enumerations = "")
    {
        _name = TagName;
        _data = Data;
        _isHeader = true; // If this instantiator is called, it must be a header.
        EnumerationItems = Enumerations;
    ...
    private void ParseToken(string TokenString)
    {
        ...
        var workingToken =
            TokenString.Trim()
                .Substring(TokenString.Trim().StartsWith("<") ? 1 : 0); // if it starts with a <, remove it.
        var workingINT = ""; // Working variable to read int values.
        var workingNDX = 0; // Index used to walk through the token string.
        ...
        if (workingToken[workingNDX] == ':')
        {
            ...
            UserDefType = workingToken[workingNDX];
            workingNDX++;
        }
        ...
        {
            if (workingToken[workingNDX] == ',' && Name.StartsWith("USERDEF"))
                break; // Stop when a comma is found, if handling a USERDEF tag.
            _data += workingToken[workingNDX];
            workingNDX++;
        }
    ...
    public override string ToString()
    {
        ...
        var tagToReturn = string.Format("<{0}:{1}{2}>{3}{4}", _name, _length,
            _isHeader && UserDefType != ' ' ? ":" + UserDefType : "",
    ...
    private void UpdateLength()
    {
        ...
        // those items plus 3.
        _length = (uint)_data.Length + (IsHeader && _name.ToUpper().StartsWith("USERDEF") && _enumerationItems != ""
            ? 3 + (uint)_enumerationItems.Length
            : 0);
    }
}